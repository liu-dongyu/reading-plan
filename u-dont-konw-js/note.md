<!--
 * @Description: 你不知道的JS-上部
 * @Author: liudongyu
 * @Date: 2019-08-27 10:01:13
 * @LastEditTime: 2019-08-27 10:58:32
...
作用域是一种查询变量的规则。词法作用域则为其中的一种工作模型，表示作用域规则会在词法分析阶段根据变量、块等的代码书写位置来制定，同时词法分析器会保持作用域不变。没有任何一个作用域能同时出现在两个外部作用域中
...
- 词法作用域中，标志符的查找使用左右查询，逐级往上
- 当多个嵌套词法作用域中存在同名标识符，只会匹配第一个遇到的，学名遮蔽效应
- 对于多级标识符，例如 a.b.c，词法作用域只会查询 1 级标识符 a，找到 a 后由对象属性访问规则接管

### 欺骗词法

> 保持词法作用域不变（根据书写时的自然关系）是最佳实践，动态改变词法作用域的手段仅供了解
...
function foo(obj) {
  // with工作机制很奇怪，会将一个对象处理为词法作用域，但该作用域内的标识符却存在于上层作用域中
  with (obj) {
    var c = 1; // c实际存在于foo的作用域中
    a = 2; // 由于a对象没有定义，左查询会一直查到顶层作用域，非严格模式下会在顶层作用域中创建a
  }
}
var o1 = {
  a: 3
...
`eval`和`with`会在运行时修改或创建新的作用域，而动态变更作用域会使得代码运行速度变慢。因为引擎会在编译阶段进行性能优化，其中某些优化依赖于词法的静态分析，当代码存在`eval`和`with`时，引擎会忽略这些优化
